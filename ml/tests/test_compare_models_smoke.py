...
def test_compare_models_smoke(tmp_path):
    ...
    out_dir = Path('ml/experiments')
    before = list(out_dir.glob('compare_*'))
    subprocess.run(['python', 'compare_models.py', '--input', str(sample.relative_to('ml')), '--epochs', '1', '--batch', '256', '--models', 'lstm,cnn'], check=True, cwd='ml')
