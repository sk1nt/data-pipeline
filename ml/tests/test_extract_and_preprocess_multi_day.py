...
def test_extract_and_preprocess_two_days(tmp_path):
    ...
    subprocess.run(['python','extract.py','--symbol',symbol,'--date',dates,'--gex-db','data/gex_data.db','--gex-ticker','NQ_NDX'], check=True, cwd=str(ML_DIR))
    file_date = '20251111'
    ...
    subprocess.run(['python','preprocess.py','--inputs',str(out1)+','+str(out2),'--window','60','--stride','1','--features','open,high,low,close,volume,gex_zero,nq_spot','--label-source','nq_spot'], check=True, cwd=str(ML_DIR))
    import glob
