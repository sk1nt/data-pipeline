...
def test_cnn_mlflow_smoke(tmp_path):
    ...
    out = Path('models/cnn_mlflow_test.pt')
    if out.exists():
        out.unlink()
    subprocess.run(['bash', '-lc', 'export MLFLOW_TRACKING_URI=http://127.0.0.1:5000 && python train_cnn.py --input ' + str(sample) + ' --epochs 1 --batch 256 --mlflow --out ' + str(out)], shell=True, check=True, cwd='ml')
    assert out.exists()
