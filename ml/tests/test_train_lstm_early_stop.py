...
def test_train_lstm_early_stop(tmp_path):
    ...
    out = Path('models/lstm_early.pt')
    if out.exists():
        out.unlink()
    subprocess.run(['python', 'train_lstm.py', '--input', str(sample), '--epochs', '5', '--batch', '256', '--dropout', '0.1', '--patience', '2', '--out', str(out)], check=True, cwd='ml')
    assert out.exists()
