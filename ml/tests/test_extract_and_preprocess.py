...
def test_extract_and_preprocess(tmp_path):
    ...
    symbol = 'MNQ'
    date = '2025-11-11'
    subprocess.run(['python', 'extract.py', '--symbol', symbol, '--date', date], check=True, cwd='ml')
    # check output exists
    file_date = '20251111'
    out_path = OUT_DIR / f"{symbol}_{file_date}_1s.parquet"
    assert out_path.exists()
    # run preprocess
    subprocess.run(['python', 'preprocess.py', '--input', str(out_path), '--window', '60', '--stride', '1'], check=True, cwd='ml')
    # check windows exist
    import glob
