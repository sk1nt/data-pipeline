...
def extract_futures_ticks(
    scid_dir: str,
    days_back: int = 70,
    parallel: bool = True,
    workers: int = 10,
    verbose: bool = False,
    max_recent_records: int = 0,
):
    """
    ...
    try:
        ...
        logger.info(
            f"Using {workers} workers, max_recent_records={max_recent_records} ({'unlimited' if max_recent_records == 0 else 'limited'}), parallel={parallel}"
        )
        ...
        if parallel:
            tick_extractor.extract_and_save_parallel(
                start_date,
                end_date,
                max_workers=workers,
                max_recent_records=actual_max_records,
            )
        else:
            ticks = tick_extractor.extract_ticks_date_range(
                start_date, end_date, actual_max_records
            )
            tick_extractor.save_ticks_to_db_optimized(ticks)

        # Extract and save depth data
        logger.info("Starting depth data extraction...")
        depth_extractor.extract_and_save_parallel(
...
class TickExtractor:
    ...
    def extract_ticks_for_date(
        self, date: datetime, max_recent_records: Optional[int] = None
    ) -> List[MnqTickRecord]:
        ...
        try:
            ...
            market_open_eastern = eastern_tz.localize(
                datetime.combine(
                    date.date(), datetime.strptime("16:15", "%H:%M").time()
                )
            )
            market_open_utc = market_open_eastern.astimezone(pytz.UTC).replace(
                tzinfo=None
            )

            logger.info(
                f"Filtering for records after {market_open_utc} (4:15 PM Eastern)"
            )

            # Load all records for this date into memory
            day_records = []
            for record in parse_scid_file_backwards_generator(
                str(scid_file), date, max_recent_records
            ):
            ...
            market_records = [
                r for r in day_records if r["timestamp"] >= market_open_utc
            ]
            ...
            logger.info(
                f"Extracted {tick_count} ticks from {len(market_records)} market records"
            )
    ...
    def _extract_ticks_from_file(
        self, scid_file: Path, date: datetime, max_recent_records: Optional[int]
    ) -> List[MnqTickRecord]:
        ...
        try:
            ...
            logger.debug(
                f"Processed {record_count} records from {scid_file.name}, extracted {tick_count} ticks"
            )
    ...
    def extract_ticks_date_range(
        self,
        start_date: datetime,
        end_date: datetime,
        max_recent_records: Optional[int] = None,
    ) -> List[MnqTickRecord]:
    ...
    def save_ticks_to_db_optimized(self, ticks: List[MnqTickRecord]):
        ...
        with db.connect() as conn:
            ...
            try:
                ...
                conn.executemany(
                    "INSERT INTO mnq_ticks (timestamp, price, volume, tick_type, ticker) VALUES (?, ?, ?, ?, ?)",
                    data,
                )
                conn.execute("COMMIT")
    ...
    def extract_and_save_parallel(
        ...
        logger.info(
            f"Starting sequential tick extraction from {start_date.date()} to {end_date.date()}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days-back", type=int, default=70, help="Number of days back to extract"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of workers for tick extraction",
    )
    parser.add_argument(
        "--depth-workers",
        type=int,
        default=1,
        help="Number of workers for depth extraction",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum recent records to process per day (0 = unlimited)",
    )
    ...
    logger.info(
        f"Using tick_workers={args.tick_workers}, depth_workers={args.depth_workers}, max_recent_records={args.max_recent_records} ({'unlimited' if args.max_recent_records == 0 else 'limited'})"
    )
    ...
    tick_extractor.extract_and_save_parallel(
        start_date,
        end_date,
        max_workers=args.tick_workers,
        chunk_size_mb=args.chunk_size,
        max_recent_records=0
        if args.max_recent_records == 0
        else args.max_recent_records,
    )

    # Extract and save depth data
    logger.info("Starting depth data extraction...")
    depth_extractor.extract_and_save_parallel(
        start_date, end_date, max_workers=args.depth_workers
    )
