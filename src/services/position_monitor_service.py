...
class PositionState:
    ...
    def unrealized_loss_ticks(self, current_price: float) -> float:
        """Positive = losing money (adverse move in ticks)."""
        if self.quantity == 0:
            return 0.0
        ...
        # Loss is positive when move is negative (for long) or positive (for short)
        loss_dollars = -move * abs(self.quantity)
        return loss_dollars / MNQ_TICK_VALUE
...
class PositionMonitorService:
    ...
    async def _handle_alert(self, alert: dict) -> None:
        ...
        new_level = 0
        if confidence >= WARNING_CONFIDENCE and loss_ticks >= WARNING_TICKS:
            new_level = 1
        if confidence >= DANGER_CONFIDENCE and loss_ticks >= DANGER_TICKS:
            new_level = 2
        if confidence >= CRITICAL_CONFIDENCE and loss_ticks >= CRITICAL_TICKS:
            new_level = 3
