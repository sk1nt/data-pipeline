...
@click.command()
@click.option('--scid-dir', required=True, type=click.Path(exists=True),
...
@click.option('--output-dir', default='data', type=click.Path(),
              help='Output directory for Parquet files')
@click.option('--parallel', is_flag=True, help='Use parallel processing for faster extraction')
@click.option('--workers', default=4, type=int, help='Number of parallel workers (default: 4)')
...
def extract_depth(scid_dir, days_back, output_dir, verbose, parallel, workers):
    ...
    try:
        ...
        if parallel:
            ...
            extractor.extract_and_save_parallel(start_date, end_date, max_workers=workers)
        else:
