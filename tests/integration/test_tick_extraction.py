...
class TestTickExtraction:
    ...
    def test_tick_validation(self):
        ...
        # Valid tick
        MnqTickRecord(timestamp=datetime(2024, 1, 1, 12, 0, 0), price=4500.12, volume=5)
        # Should not raise

        # Invalid price
        with pytest.raises(ValueError, match="Price must be positive"):
            MnqTickRecord(
                timestamp=datetime(2024, 1, 1, 12, 0, 0), price=-100, volume=5
            )

        # Invalid volume
        with pytest.raises(ValueError, match="Volume must be positive"):
            MnqTickRecord(
                timestamp=datetime(2024, 1, 1, 12, 0, 0), price=4500.12, volume=-1
            )
