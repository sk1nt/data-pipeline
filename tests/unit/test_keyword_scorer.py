...
class TestKeywordScorer:
    def test_tariff_tweet_scores_high(self):
        ...
        score, keywords, categories, _ = scorer.score(
    ...
    def test_case_insensitive(self):
        scorer = KeywordScorer()
        score1, _, _, _ = scorer.score("TARIFF")
        score2, _, _, _ = scorer.score("tariff")
        score3, _, _, _ = scorer.score("Tariff")
    ...
    def test_word_boundary_matching(self):
        ...
        score, keywords, _, _ = scorer.score("I fed the cat today")
    ...
    def test_custom_keyword_config(self):
        ...
        score, keywords, categories, _ = scorer.score("This stock is a moonshot!")
        assert score == 3
        assert "moonshot" in keywords
        assert "custom" in categories

        # Default keywords should NOT match
        score2, _, _, _ = scorer.score("tariff on China")
        assert score2 == 0

    def test_threshold_filtering(self):
        scorer = KeywordScorer()
        # Low-weight match only (crypto = weight 1)
        score, _, _, _ = scorer.score("Bitcoin price is rising")
        assert score >= 1
