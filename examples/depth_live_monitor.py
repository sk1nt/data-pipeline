...
DEFAULT_DEPTH_ROOT = Path(
    os.getenv("SIERRA_DEPTH_DIR", "/mnt/c/SierraChart/Data/MarketDepthData")
)
...
def parse_depth_record(record_bytes: bytes) -> Dict:
    ...
    try:
        ...
        dt_raw, cmd, flags, num_orders, price, qty, reserved = struct.unpack(
            "<QBBHfII", record_bytes
        )
...
def find_sierra_chart_depth_file(
    symbol: str, search_root: Optional[Path] = None
) -> Optional[Path]:
        ...
        try:
            ...
            if current_size > last_size and current_size > DEPTH_HEADER_SIZE:
                        ...
                        if record and record["price"] > 0:
                            ...
                            if record["side"] == "BUY" or record["command"] in [1, 2]:
                                if size > 0:
                                    ...
                                    bids.pop(price, None)
                            elif record["side"] == "SELL" or record["command"] in [
                                3,
                                4,
                            ]:
                                if size > 0:
                            ...
                            print(
                                f"[{timestamp}] {symbol} | Price: ${price:.2f} | Size: {size} | "
                ...
                if bids and asks:
                    ...
                    print(
                        f"   📊 Best Bid: ${best_bid:.2f} ({bids[best_bid]}) | "
...
def main():
    parser = argparse.ArgumentParser(
        description="Real-time Sierra Chart depth monitoring"
    )
    parser.add_argument(
        "--symbol", required=True, help="Symbol to monitor (ES, MNQ, etc.)"
    )
    parser.add_argument(
