...
async def main():
    ...
    print(
        f"ThinkorSwim: {'Enabled' if config.thinkorswim_allocation.enabled else 'Disabled'} ({config.thinkorswim_allocation.percentage}%)"
    )
    print(
        f"TastyTrade: {'Enabled' if config.tastytrade_allocation.enabled else 'Disabled'} ({config.tastytrade_allocation.percentage}%)"
    )

    if config.tastytrade_credentials:
        env_label = (
            "sandbox"
            if config.tastytrade_credentials.use_sandbox_environment
            else "cert"
            if config.tastytrade_credentials.use_cert_environment
            else "live"
        )
        print(
            f"TastyTrade credentials loaded ({'dry run' if config.tastytrade_credentials.dry_run else 'live'}) [{env_label}]"
        )
        if config.tastytrade_credentials.account_whitelist:
            print(
                f"TastyTrade account whitelist: {', '.join(config.tastytrade_credentials.account_whitelist)}"
            )
        if config.tastytrade_credentials.default_account:
            print(
                f"TastyTrade default account: {config.tastytrade_credentials.default_account}"
            )
    else:
    ...
    if config.allowed_channel_ids:
        print(
            f"Allowed Discord channels: {', '.join(str(cid) for cid in config.allowed_channel_ids)}"
        )
