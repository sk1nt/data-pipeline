...
settings, RedisClient, RedisTimeSeriesClient, FlushWorkerSettings, RedisFlushWorker = (
    _load_flush_dependencies()
)
...
async def main() -> None:
    ...
    worker = RedisFlushWorker(
        redis_client, ts_client, FlushWorkerSettings(interval_seconds=5)
    )
    worker.start()
