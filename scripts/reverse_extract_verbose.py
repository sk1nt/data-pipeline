...
def parse_args():
    ...
    p.add_argument(
        "--parquet-dir", default="data/depth_parquet", help="Parquet output directory"
    )
    p.add_argument(
        "--clean", action="store_true", help="Remove existing DB tables before running"
    )
    p.add_argument(
        "--max-per-day", type=int, default=0, help="Limit ticks per day (0 = unlimited)"
    )
    p.add_argument(
        "--top-k",
        type=int,
        default=80,
        help="Top K bid/ask levels to keep per snapshot",
    )
    p.add_argument(
        "--depth-chunk-size",
        type=int,
        default=100000,
        help="Number of snapshots per parquet chunk",
    )
    p.add_argument(
        "--emit-tick-parquet",
        action="store_true",
        help="Write ticks per-day to parquet instead of inserting into DuckDB",
    )
    p.add_argument(
        "--ticks-parquet-dir",
        default="data/parquet/ticks",
        help="Output dir for per-day ticks parquet files",
    )
    p.add_argument(
        "--tick-chunk-size",
        type=int,
        default=50000,
        help="Number of tick rows per parquet write chunk",
    )
    ...
    try:
        while current >= start_date:
            ...
            if os.path.exists(depth_path):
                log.info(
                    "  Streaming depth file into chunks: %s -> %s",
                    depth_path,
                    day_out_dir,
                )
                ...
                if rows_chunk:
                    ...
                    writer.write_table(tbl)
                    log.info(
                        "  Appended final chunk to %s (%d rows)",
                        out_file,
                        len(rows_chunk),
                    )
                    rows_chunk = []
                ...
                log.info(
                    "  Parsed and wrote %d snapshots for %s (%.2fs) -> %s",
                    len(snaps_timestamps),
                    day_str,
                    parse_time,
                    out_file,
                )
            ...
            if not snaps_timestamps:
                ...
                for record in parse_scid_file_backwards_generator(
                    scid_file,
                    date_filter=current,
                    max_records=(args.max_per_day or None),
                ):
                    ts = record["timestamp"]
                    ...
                    if len(buffer) >= args.batch_size:
                        ...
                        inserted += len(buffer)
                        log.info(
                            "  Inserted batch %d rows (%.2f rows/s)",
                            len(buffer),
                            len(buffer) / max(dt, 1e-6),
                        )
                        buffer = []
                if buffer:
                    t_ins = time.time()
                    ...
                    inserted += len(buffer)
                    log.info(
                        "  Inserted final batch %d rows (%.2f rows/s)",
                        len(buffer),
                        len(buffer) / max(dt, 1e-6),
                    )
                else:
                    ...
                    for record in parse_scid_file_backwards_generator(
                        ...
                        while (
                            snap_idx < len(snaps_timestamps)
                            and ts < snaps_timestamps[snap_idx]
                        ):
                            if buffered:
                                t_ins = time.time()
                                conn.executemany(
                                ...
                                log.info(
                                    "  Inserted %d buffered ticks for snapshot %d (%.2f rows/s)",
                                    len(buffered),
                                    snap_idx + 1,
                                    len(buffered) / max(dt, 1e-6),
                                )
                                buffered = []
                            snap_idx += 1
                    ...
                    if buffered:
                        ...
                        inserted += len(buffered)
                        log.info(
                            "  Inserted final buffered %d rows (%.2f rows/s)",
                            len(buffered),
                            len(buffered) / max(dt, 1e-6),
                        )
                    ...
                    log.info(
                        "  Day %s: ticks processed=%d, inserted=%d, time=%.1fs, rate=%.1f rows/s",
                        day_str,
                        tick_count,
                        inserted,
                        elapsed,
                        (inserted / max(elapsed, 1e-6)),
                    )
                    ...
                    if snaps_timestamps:
                        ...
                        conn.execute(
                            "INSERT OR REPLACE INTO mnq_depth_metadata (file_path, date_range_start, date_range_end, record_count) VALUES (?, ?, ?, ?)",
                            [
                                metadata_path,
                                date_range_start,
                                date_range_end,
                        ...
                        log.info(
                            "  Registered depth parquet %s rows=%d range=%s->%s",
                            metadata_path,
                            record_count,
                            date_range_start,
                            date_range_end,
                    ...
                    log.info(
                        "  Wrote depth parquet %s (%d rows) range=%s -> %s",
                        out_file,
                        record_count,
                        date_range_start,
                        date_range_end,
