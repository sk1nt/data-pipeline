...
def get_scid_df_cached(scid_file: str, limitsize: int = sys.maxsize) -> pl.DataFrame:
    ...
    df = pl.DataFrame(
        {
            'timestamp': pl.from_epoch(unix_ms, time_unit='ms')
...
def load_gex_day_df(gex_file: str) -> pl.DataFrame:
    ...
    gex_df = pl.DataFrame(processed, schema=schema, strict=False).with_columns(
...
def load_gex_record(gex_file, target_timestamp):
    ...
    match = (
        gex_df.filter(pl.col('timestamp').round(0) == int(target_timestamp))
        .select(pl.all())
    )
    if match.height == 0:
...
def _build_1s_bars(scid_df: pl.DataFrame, start_utc: datetime, end_utc: datetime) -> pl.DataFrame:
    ...
    return (
        filtered_df
        .with_columns(pl.col('timestamp').dt.truncate('1s').alias('bar_timestamp'))
...
def _join_gex(df_bars: pl.DataFrame, gex_df: pl.DataFrame | None) -> pl.DataFrame:
    ...
    gex_join = gex_df.with_columns(
        pl.col('gex_timestamp').dt.cast_time_unit('ms')
    )
    df_join = df_bars.with_columns(
        pl.col('bar_timestamp').dt.cast_time_unit('ms')
    )
...
def enrich_day(date_str: str, scid_df: pl.DataFrame, scid_label: str) -> Path | None:
    ...
    df_with_gex = _join_gex(df_1s_bars, gex_df).with_columns(pl.lit('MNQ').alias('symbol'))
    ...
    tick_columns = ['symbol', 'bar_timestamp', 'time_et', 'Open', 'High', 'Low', 'Close', 'TotalVolume', 'BidVolume', 'AskVolume', 'NumTrades']
...
def enrich_single_timestamp(timestamp: float, date_str: str, scid_df: pl.DataFrame, scid_label: str) -> Path | None:
    ...
    if not (rth_start_utc <= start_utc < rth_end_utc):
        print(f'Error: timestamp {timestamp} ({start_utc}) is outside RTH for {date_str} (09:30-16:00 ET)')
        return None
...
def main():
    ...
    mode.add_argument('--timestamp', type=float, help='Unix timestamp (seconds) to analyze')
    mode.add_argument('--date', type=str, help='Single day YYYY-MM-DD')
    mode.add_argument('--date-range', nargs=2, metavar=('START_DATE', 'END_DATE'), help='Inclusive day range YYYY-MM-DD YYYY-MM-DD')
    ...
    if args.timestamp is not None:
        ...
        enrich_single_timestamp(args.timestamp, args.timestamp_date, scid_df, args.scid_file)
        return
