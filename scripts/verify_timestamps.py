...
def check_file(path: Path, kind: str, dt: date):
    ...
    try:
        ...
        mismatch_q = (
            f"SELECT COUNT(*) FROM read_parquet('{path}') WHERE CAST(timestamp AS DATE) != DATE('{dt.isoformat()}')"
        )
        ...
        if has_ts_ms:
            ...
            min_ts = con.execute(f"SELECT MIN(timestamp) FROM read_parquet('{path}')").fetchone()[0]
            max_ts = con.execute(f"SELECT MAX(timestamp) FROM read_parquet('{path}')").fetchone()[0]
            min_ts_ms = con.execute(f"SELECT MIN(ts_ms) FROM read_parquet('{path}')").fetchone()[0]
            max_ts_ms = con.execute(f"SELECT MAX(ts_ms) FROM read_parquet('{path}')").fetchone()[0]
...
def main():
    ...
    for dt in daterange(start_dt, end_dt):
        for kind in (["depth"] if args.kind == "depth" else (["tick"] if args.kind == "tick" else ["depth", "tick"])):
            path = fmt_path(kind, args.symbol, dt)
            if not path.exists():
                rows.append({"kind": kind, "path": str(path), "date": dt.isoformat(), "exists": False})
                continue
    ...
    corrupt = [r for r in rows if not r.get("readable", False) and r.get("exists", False)]
    missing = [r for r in rows if not r.get("exists", False)]
    missing_ts_ms = [r for r in rows if r.get("exists") and r.get("readable") and not r.get("has_ts_ms")]
    mismatch_date = [r for r in rows if r.get("exists") and r.get("readable") and r.get("mismatch_rows", 0) > 0]
    ts_ms_mismatch = [r for r in rows if r.get("exists") and r.get("readable") and r.get("ts_ms_mismatch_rows", 0) > 0]
    ...
    if args.out_csv:
        ...
        with open(out, "w", newline="", encoding="utf-8") as fh:
            writer = csv.DictWriter(fh, fieldnames=["kind","path","date","exists","readable","rows","has_ts_ms","mismatch_rows","ts_ms_mismatch_rows","min_ts","max_ts","min_ts_ms","max_ts_ms","error"])# noqa: E501
            writer.writeheader()
            for r in rows:
    ...
    if missing_ts_ms:
        ...
        for r in missing_ts_ms[:20]:
            print(r.get("path"), "rows:", r.get("rows"), "min_ts:", r.get("min_ts"), "max_ts:", r.get("max_ts"))
    if mismatch_date:
        print("\nDATE(timestamp) mismatch samples:")
        for r in mismatch_date[:20]:
            print(r.get("path"), "mismatch_rows:", r.get("mismatch_rows"), "rows:", r.get("rows"), "min_ts:", r.get("min_ts"), "max_ts:", r.get("max_ts"))
    if ts_ms_mismatch:
        print("\nTs_ms mismatch samples:")
        for r in ts_ms_mismatch[:20]:
            print(r.get("path"), "ts_ms_mismatch_rows:", r.get("ts_ms_mismatch_rows"), "rows:", r.get("rows"))
