...
async def get_realtime_ticks(
    ...
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of ticks to return per symbol"
    ),
    ...
    logger.info(
        f"Real-time query from model {model_id} for symbols {symbols}, limit {limit}"
    )
    ...
    logger.info(
        f"Real-time query completed in {response_time}ms, returned {len(ticks)} ticks"
    )
...
async def get_historical_ticks(
    ...
    interval: str = Query(
        "1h", pattern="^(1s|1m|1h|4h)$", description="Aggregation interval"
    ),
    model_id: str = Depends(verify_api_key),
):
    ...
    logger.info(
        f"Historical query from model {model_id} for symbols {symbols}, interval {interval}"
    )
    ...
    logger.info(
        f"Historical query completed in {response_time}ms, returned {len(data)} data points"
    )
