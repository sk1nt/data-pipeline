...
def test_convert_parquet_to_ts_ms_with_tz(tmp_path):
    ...
    cmd = [sys.executable, "scripts/convert_parquet_to_ts_ms.py", str(p), "--out", str(out), "--atomic", "--timestamp-tz", "America/New_York"]
    ...
    rows = con.execute(f"SELECT ts_ms, CAST(EXTRACT(epoch FROM timestamp AT TIME ZONE 'America/New_York') * 1000 AS BIGINT) FROM read_parquet('{out}')").fetchall()
...
def test_verify_timestamps_script_no_mismatch(tmp_path):
    ...
    df = pl.DataFrame({"timestamp": [dt_utc], "ts_ms": [int(dt_utc.timestamp() * 1000)]})
    df.write_parquet(p)

    cmd = [sys.executable, "scripts/verify_timestamps.py", "--start", "2025-11-11", "--end", "2025-11-11", "--symbol", "MNQ", "--parquet-root", str(tmp_path.parent), "--kind", "depth", "--timestamp-tz", "UTC"]
    ...
    cmd = [sys.executable, "scripts/verify_timestamps.py", "--start", "2025-11-11", "--end", "2025-11-11", "--symbol", "MNQ", "--parquet-root", str(tmp_path / "data" / "parquet"), "--kind", "depth", "--timestamp-tz", "UTC"]
    proc = subprocess.Popen(cmd)
    rc = proc.wait()
